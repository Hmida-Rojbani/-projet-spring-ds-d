...
public class GroupSubjectServiceImp implements GroupSubjectService {
    ...
    @Override
    public Subject findSubjectWithMaxAbsence(List<GroupSubject> groupSubjects){
        Subject subjectMaxAbsence = null;
        float actual=0;
        float max=0;
        for (GroupSubject groupSubject:groupSubjects) {
            actual=absenceService.hoursCountByGroupAndSubject(groupSubject.getGroup().getId(), groupSubject.getSubject().getId());
            if(actual>max){
                max=actual;
                subjectMaxAbsence=groupSubject.getSubject();
            }
        }
        return subjectMaxAbsence;
    }
    public Subject findSubjectWithMinAbsence(List<GroupSubject> groupSubjects){
        Subject subjectMinAbsence = null;
        float actual=0;
        float min=99999;
        for (GroupSubject groupSubject:groupSubjects) {
            actual=absenceService.hoursCountByGroupAndSubject(groupSubject.getGroup().getId(), groupSubject.getSubject().getId());
            if(actual<min){
                min=actual;
                subjectMinAbsence=groupSubject.getSubject();
            }
        }
        return subjectMinAbsence;
    }
